...
def main():
    ...
    trn_dataset = Longitudinal_Dataset(args.data_dir, subset=args.data_subset, data_type=args.data_type, outcome_type=args.progression_outcome, modality=args.data_modality, resolution=args.image_size)
    val_dataset = Longitudinal_Dataset(args.data_dir, subset=args.data_val_subset, outcome_type=args.progression_outcome, modality=args.data_modality,  resolution=args.image_size)
    tst_dataset = Longitudinal_Dataset(args.data_dir, subset=args.data_tst_subset, outcome_type=args.progression_outcome, modality=args.data_modality,  resolution=args.image_size)
    ...
    in_dim = args.data_modality
    out_dim = 1
    vf_predictor = create_model(model_type=args.model, in_dim=in_dim, out_dim=out_dim)
    ...
    pseudo_supervisor = create_model(model_type=args.model, in_dim=in_dim, out_dim=out_dim)
    ...
    optimizer_policy = AdamW(pseudo_supervisor.model_params, lr=args.lr_policy, betas=(0.0, 0.1), weight_decay=args.weight_decay_vf) # amsgrad=True, eps=1e-08, 
...
def create_argparser():
    defaults = dict(
        ...
        data_modality=1,
        progression_outcome='progression_outcome_md_fast_no_p_cut',
